...
CREATE TABLE IF NOT EXISTS `SIDEDISH_ORDER` (
  ...
  PRIMARY KEY (`ORDER_USERS_ID`, `ORDER_MENU_ID`),
  INDEX `fk_SIDEDISH_USERS_has_SIDEDISH_MENUS_SIDEDISH_MENUS1_idx` (`ORDER_MENU_ID` ASC) VISIBLE,
  INDEX `fk_SIDEDISH_USERS_has_SIDEDISH_MENUS_SIDEDISH_USERS1_idx` (`ORDER_USERS_ID` ASC) VISIBLE,
  CONSTRAINT `fk_SIDEDISH_USERS_has_SIDEDISH_MENUS_SIDEDISH_USERS1`
    FOREIGN KEY (`ORDER_USERS_ID`)
    REFERENCES `sidedish`.`SIDEDISH_USERS` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_SIDEDISH_USERS_has_SIDEDISH_MENUS_SIDEDISH_MENUS1`
