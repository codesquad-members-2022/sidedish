...
const HeaderWrap = styled.header`
  ${({ theme }) => theme.custom_absolute(20, null, null, 20) + theme.width_height_bypx(1440, 83)}
  border-bottom: 1px solid ${({ theme }) => theme.colors.Black};
  background: ${({ theme }) => theme.colors.White};
`;

const HeaderLogo = styled.span`
  ${({ theme }) => theme.custom_absolute(16, null, null, 80) + theme.width_height_bypx(174, 50)}
  ${Logo};
`;
const GNB = styled.div`
  ${({ theme }) =>
    theme.custom_flex(`row`, `flex-start`, `center`) + theme.custom_absolute(28, null, null, 294)}
  padding: 0px;
  ...
  li {
    ...
    ${({ theme, index }) =>
      theme.custom_flex("column", "flex-start") + theme.custom_static(0) + theme.flex_none(index, 0, 24)};
  }
    padding: 0px;
    width: auto;
    height: 26px;
  ...
  return (
    <HeaderWrap>
      ...
      <GNB onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
        {menus.map((menu) => (
          <GnbMenu cats={menu.cats} show={show}></GnbMenu>
        ))}
      </GNB>
      <Icon>
        <IconBox index={0}>
          <Search width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
        ...
        <IconBox index={1}>
          <MyPage width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
